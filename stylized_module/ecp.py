...
class EcpMod(object):
    ...
    """
    def __init__(self,cell,electrode_positions,move_cell=None,scale=1.0):
        """
        cell: cell object
        electrode_positions: n-by-3 array of electrodes coordinates
        move_cell, scale: see method 'calc_transfer_resistance'
        """
        self.cell = cell
        ...
        self.record_im()
        self.calc_transfer_resistance(cell.seg_coords,move_cell,scale)
    ...
    def calc_transfer_resistance(self,seg_coords,move_cell,scale):
        """
        Precompute mapping from segment to electrode locations
        seg_coords: segment coordinates from the cell object
        ...
        for j in range(self.nelec):  # calculate mapping for each site on the electrode
            ...
            up = rll + dlmag / 2
            low = rll - dlmag / 2
            np.fmax(rT2, d ** 2, out=rT2, where= low - d < 0)
